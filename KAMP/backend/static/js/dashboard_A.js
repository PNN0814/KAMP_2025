// Tab: Preprocessing 1
createPredictionChart([92, 87, 95, 91, 88, 94, 89, 96, 93, 90]);
createFeatureImportanceChart([0.25, 0.18, 0.15, 0.12, 0.10]);
createEpochsChart([0.75, 0.82, 0.88, 0.92, 0.95, 0.97, 0.98]);
createConfusionMatrix();

// Preprocessing Specific Charts
function createPredictionChart(actualData) {
    const ctx = document.getElementById(`chart-prep1-prediction`);
...
function createFeatureImportanceChart(importanceData) {
    const ctx = document.getElementById(`chart-prep1-features`);
    new Chart(ctx, {
        type: 'bar',
        data: {
...
function createEpochsChart(accuracyData) {
    const ctx = document.getElementById(`chart-prep1-epochs`);
    new Chart(ctx, {
        type: 'line',
        data: {
...
function createConfusionMatrix() {
    const ctx = document.getElementById(`chart-prep1-confusion`);
    new Chart(ctx, {
        type: 'bar',
        data: {
